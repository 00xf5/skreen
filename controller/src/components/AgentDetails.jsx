...
export function AgentDetails({ agent, onBack, onTogglePersistence, onStartScreen, onOpenProcesses, onOpenFiles, onUninstall, results, sendCommand }) {
  ...
  // Derive privilege level directly from agent metadata — no hardcoding
  const privilege = agent?.privilege || 'user'
  ...
  const privLabel = privilege === 'system'
    ? { icon: '🔴', text: 'SYSTEM', cls: 'system' }
    : privilege === 'admin'
    ? { icon: '🟡', text: 'Administrator', cls: 'admin' }
    : { icon: '🟢', text: 'Standard User', cls: 'user' }
      ...
      case 'summary':
        return (
          <div className="details-grid">
            ...
            <div className="detail-card">
              <h4>Networking</h4>
              <div className="detail-row"><span>Local IP</span><span className="val-bright">{stats.local_ip || '—'}</span></div>
              <div className="detail-row"><span>Public IP</span><span className="val-bright">{stats.public_ip || 'Fetching...'}</span></div>
              <div className="detail-row"><span>Username</span><span className="val-bright">{agent?.username || '—'}</span></div>
            </div>

            <div className="detail-card">
              <h4>Privileges</h4>
              <div className="privilege-indicator">
                <span className={`priv-badge ${privLabel.cls}`}>
                  {privLabel.icon} {privLabel.text}
                </span>
              </div>
              <p className="detail-note">
                {privilege === 'admin' || privilege === 'system'
                  ? 'Agent has elevated privileges. Full control available.'
                  : 'Agent runs as standard user. Some operations may require elevation.'}
              </p>
            </div>
            ...
            <div className="detail-card danger">
              ...
              <div className="danger-actions">
                <button
                  ...
                  onClick={() => window.confirm('Permanently uninstall this agent? This will remove registry persistence and delete the binary on next reboot.') && onUninstall(agent.id)}
                >
                  Uninstall Agent
                </button>
              </div>
          ...
          <div className="toolbox-view">
            <div className="toolbox-grid">
              {tools.map(tool => (
                <button
                  key={tool.id}
                  className={`tool-card ${runningTool === tool.id ? 'running' : ''}`}
            ...
            <div className="custom-tool">
              ...
              <div className="custom-input-group">
                <input
                  type="text"
                  placeholder="Enter command or script (e.g. dir /s, powershell Get-Process)..."
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && e.target.value.trim()) {
                      runTool({ id: 'custom', name: 'Custom Script', cmd: e.target.value })
                      e.target.value = ''
                    }
                  }}
                />
                <button onClick={(e) => {
                  ...
                  if (input.value.trim()) {
                    runTool({ id: 'custom', name: 'Custom Script', cmd: input.value })
                    input.value = ''
                  }
                }}>Run</button>
              </div>
            ...
            {toolResult && (
              <div className={`tool-output ${toolResult.status}`}>
                <div className="output-header">
                  <span>Result: {toolResult.tool}</span>
                  <button onClick={() => setToolResult(null)}>✕ Clear</button>
                </div>
